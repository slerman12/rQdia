...
from SPR_rQdia.src.utils import count_parameters, dummy_context_mgr
import numpy as np
...
class SPRCatDqnModel(torch.nn.Module):
    ...
    def forward(self, observation,
        ...
        if train:
            ...
            latent_orig = self.stem_forward(input_obs,
                                       prev_action[0],
                                       prev_reward[0])
            ...
            log_pred_ps.append(self.head_forward(latent,
                                                 ...
                                                 logits=True))
            if self.use_rqdia:
                rqdia_loss = F.mse_loss(log_pred_ps[-1], self.head_forward(latent_orig,
                                                                       prev_action[0],
                                                                       prev_reward[0],
                                                                       logits=True))
            else:
                rqdia_loss = torch.zeros(1)
            ...
            if self.use_spr:
                spr_loss = self.do_spr_loss(pred_latents, observation) + rqdia_loss
            else:
